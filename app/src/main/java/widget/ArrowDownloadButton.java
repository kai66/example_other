...
public class ArrowDownloadButton extends View {
    ...
    private int measure(int measureSpec, boolean isWidth) {
        ...
        Log.v("TAG","ArrowDownloadButton01 mode="+mode+" size="+size+"  isWidth="+isWidth);
        int padding = isWidth ? getPaddingLeft() + getPaddingRight() : getPaddingTop() + getPaddingBottom();
        if (mode == MeasureSpec.EXACTLY) {
            Log.v("TAG","ArrowDownloadButton002 ");
            result = size;
        } else {
            result = isWidth ? getSuggestedMinimumWidth() : getSuggestedMinimumHeight();
            result += padding;
            Log.v("TAG","ArrowDownloadButton02 mode="+mode+" size="+size+"  result="+result);
